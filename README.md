...
To obtain these values, follow the steps in [Optional -- Prepare Azure OpenAI for use](#optional----prepare-azure-openai-for-use).
...
```bash
./mvnw liberty:run -Popenliberty-pathtraversal-local > YYYYMMDD-liberty-log-NN.txt 2>&1
```
...
curl -s -X GET -H "Accept: application/json" \
  "http://localhost:9080/cargo-tracker/rest/graph-traversal/shortest-path?origin=CNHKG&destination=USNYC" | jq .
```
